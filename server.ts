...
const stmt = {
  list:   db.prepare<[], Note>       ("SELECT * FROM notes ORDER BY created_at DESC"),
  get:    db.prepare<[string], Note> ("SELECT * FROM notes WHERE id = ?"),
  insert: db.prepare<[string, string, string, string], void>(
            "INSERT INTO notes (id, title, content, created_at) VALUES (?, ?, ?, ?)"),
  update: db.prepare<[string, string, string, string], void>(
            "UPDATE notes SET title = ?, content = ?, updated_at = ? WHERE id = ?"),
  delete: db.prepare<[string], void> ("DELETE FROM notes WHERE id = ?"),
};
...
function listNotes(_req: IncomingMessage, res: ServerResponse): void {
  const notes = stmt.list.all();
  send(res, 200, notes);
}

function getNote(_req: IncomingMessage, res: ServerResponse, id: string): void {
...
async function updateNote(req: IncomingMessage, res: ServerResponse, id: string): Promise<void> {
...
function deleteNote(_req: IncomingMessage, res: ServerResponse, id: string): void {
  const existing = stmt.get.get(id);
  if (!existing) return send(res, 404, { error: "Note not found" });
